...
NODE *insert_in_leaf(NODE *leaf, int key)
{
    int i;
    if (key < leaf->key[0])
    {
...
void split(int key, TEMP *tnode, NODE *leaf)
{
    int i;
    i = slide_element_in_tempnode(tnode, key);
...
int main(int argc, char *argv[])
{
    ...
    for (i = 0; i < NUM_THREAD; i++)
    {
        ...
        data[i].num = SIZE / NUM_THREAD;
        data[i].key = data[i].num * i - 1;
    }
    ...
    {
        pthread_create(&thread[i], NULL, insert, &data[i]);
    }
